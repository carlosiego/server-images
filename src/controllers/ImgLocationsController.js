// const ImgLocationsRepository = require('../repositories/ImgLocationsRepository')
// const HandleImageServer = require('../HandleImageServer/index')

// class ImgLocationsController {
...
// 		let imageUpdated = await ImgLocationsRepository.create({ name, size, storehouse, street, side, shelf, column, description, code })
...
// 		let images = await ImgLocationsRepository.findByCode(code)
// 		let imagesWithPath = []
...
// 		let imageExists = await ImgLocationsRepository.findByName(nameCurrent)
...
// 			imageUpdated = await ImgLocationsRepository.updateAll({
// 				nameCurrent, filename, code, size, storehouse, street, side, shelf, column, description
// 			})
// 		} else {
// 			imageUpdated = await ImgLocationsRepository.update({
...
// 		let image = await ImgLocationsRepository.findByName(name)

// 		if (!image) return res.status(400).json({ error: 'Não existe imagem com o nome ' + name })

// 		Promise.all([
// 			ImgLocationsRepository.deleteByName(name),
...
// module.exports = new ImgLocationsController()
